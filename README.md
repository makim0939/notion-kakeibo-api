...
月次サマリページの内容を、プロパティの値から組み立て直す。

| クエリ | 必須 | 説明 |
| --- | --- | --- |
| `pageid` | ✅ | サマリページのID。ハイフン有無どちらも可 |
| `key` |  | API キー。ヘッダを付けられないブラウザ経路向け（`X-API-Key` ヘッダでも可） |

NotionDB「家計簿」の数式プロパティ `🔄️サマリ更新` が
`https://<worker>/summary?pageid=` + `id()` を出すので、それをクリックして使う。
結果は人が読める HTML で返るため、そのままブラウザで確認できる。

`key` をクエリで渡せるようにしているのは、Notion からリンクを開く経路ではリクエストヘッダを付けられないため。
数式を `... + "&key=<API_KEY>"` に変えると、クリックだけで完結する。

### `POST /summary`

同じ処理を JSON で返す。`pageid` はクエリかボディで渡す。
