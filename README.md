...
ページ本文を直接書き換えるのではなく、**目印のコールアウト1つとその子ブロックだけ**を管理する。

- 初回はページ末尾にコールアウト「月次サマリ（自動生成）」を作成する。
- 2回目以降は同じコールアウトを見つけ、子ブロックを作り直す。何度実行しても重複しない。
- **生成結果が現在の内容と同じなら、何も書かない。** 定期実行が空振りしても
  Notion の更新履歴は汚れず、「最終更新」は実際に中身が変わった時刻を指す。
- コールアウトの外にあるユーザ作成のブロック（目標のチェックボックス、カテゴリ別DBなど）には触れない。
- コールアウトを好きな位置にドラッグすれば、以降その位置が保たれる。
...
- 資産推移（総資産・貯金口座・積立投信の前月比）
- 収支（総収入・総支出・差額）
- **カテゴリ別の支出**（カテゴリ / 件数 / 金額 / 構成比 / 前月比の表）
- **支出の明細**（カテゴリごとにまとめ、各カテゴリ内は金額の大きい順。カテゴリ小計つき）

支出の集計は「購入日」が対象月に入る支出を支出データソースから引いて行う。
前月分も読み、カテゴリごとの増減を出す。支出が読めなかった場合はこの2セクションを省き、
資産推移などは通常どおり出す。

### リンクドDBビューではなく表を描いている理由

Notion API では、ページに**リンクドデータベースビューを作成できない**。
ブロック作成の型に `child_database` / `linked_database` が無く、`views.create` も
親がデータベースなので「DBに属するビュー」しか作れない。
そのため、グルーピング・フィルタ・合計を適用したビューの代わりに、
同じ内容を Notion のテーブルブロックとして描画している。
